...
import ru.megains.ilos.item.ItemType
...
class Player(val id: Int) extends Logger[Player] {
    ...
    var skills: Skills = new Skills(this)
    ...
    def useItem(id: Int): Unit = {
        inventory.getItem(id) match {
            case Some(itemStack) =>
                itemStack.item.itemType match {
                    case ItemType.material =>

                    case ItemType.book =>
                       val success = itemStack.item.src match {
                            case "books-23" =>
                                skills.learnSkill(SkillClass.mine)
                            case "books-68" =>
                                skills.learnSkill(SkillClass.forest)
                            case "books-92" =>
                                skills.learnSkill(SkillClass.meadow)
                        }
    ...
    def sendPlayerInfo(): Unit = {
        sendPacket(new PacketPlayerInfo(this))
        sendPacket(new PacketPlayerSkills(this))
